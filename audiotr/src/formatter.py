...
class MarkdownFormatter:
    ...
    def __init__(
        ...
        Args:
            ...
            include_metadata: Include audio/transcription metadata.
    ...
    def format(
        ...
        Args:
            ...
            audio_info: Optional audio metadata dictionary.

        Returns:
            Formatted markdown string.
        """
        if self.style == MarkdownStyle.SIMPLE:
            return self._format_simple(result, audio_path, audio_info)
        elif self.style == MarkdownStyle.TIMESTAMPED:
            return self._format_timestamped(result, audio_path, audio_info)
        elif self.style == MarkdownStyle.DETAILED:
            return self._format_detailed(result, audio_path, audio_info)
        elif self.style == MarkdownStyle.SRT_STYLE:
            return self._format_srt_style(result, audio_path, audio_info)
        else:
            return self._format_timestamped(result, audio_path, audio_info)
    ...
    def _format_header(
        ...
        if self.include_metadata:
            ...
            if audio_path:
                lines.append(f"- **Source File:** `{Path(audio_path).name}`")

            if audio_info:
                if audio_info.get("duration_formatted"):
                    lines.append(f"- **Duration:** {audio_info['duration_formatted']}")
                if audio_info.get("format"):
                    lines.append(f"- **Format:** {audio_info['format']}")
    ...
    def _format_simple(
        ...
        if self.include_metadata:
            lines.append(self._format_header(audio_path, audio_info, result))
