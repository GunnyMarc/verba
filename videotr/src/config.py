"""
Configuration manager for videotr.

Handles reading, writing, and prompting for videotr.conf settings.
"""
...
CONF_FILE = CONF_DIR / "videotr.conf"
...
def ensure_conf_dir_and_file():
    """Ensure conf directory and videotr.conf exist, creating them if needed."""
    created_dir = False
    created_file = False
...
def prompt_directories(config: configparser.ConfigParser) -> configparser.ConfigParser:
    ...
    for key, label in [
        ("input_dir", "Input directory (source video files)"),
...
def display_config(config: configparser.ConfigParser):
    ...
    print("")
    print("=" * 55)
    print("  videotr Configuration (videotr.conf)")
    print("=" * 55)
...
def initialize_config(cli_model: Optional[str] = None,
    ...
    if config_is_empty(config):
        print("No configuration found. Let's set up videotr.")
...
def configure_settings(config: configparser.ConfigParser) -> configparser.ConfigParser:
    """
    Walk through every entry in videotr.conf and let the user change values.
    ...
    print("")
    print("=" * 55)
    print("  videotr Configuration Editor")
    print("=" * 55)
