"""
Configuration manager for streamtr.

Handles reading, writing, and prompting for streamtr.conf settings.
"""
...
CONF_FILE = CONF_DIR / "streamtr.conf"
...
def ensure_conf_dir_and_file():
    """Ensure conf directory and streamtr.conf exist, creating them if needed."""
    created_dir = False
    created_file = False
...
def prompt_directories(config: configparser.ConfigParser) -> configparser.ConfigParser:
    ...
    for key, label in [
        ("input_dir", "Input directory (for batch URL files)"),
...
def display_config(config: configparser.ConfigParser):
    ...
    print("")
    print("=" * 55)
    print("  streamtr Configuration (streamtr.conf)")
    print("=" * 55)
...
def initialize_config(cli_model: Optional[str] = None,
    ...
    if config_is_empty(config):
        print("No configuration found. Let's set up streamtr.")
...
def configure_settings(config: configparser.ConfigParser) -> configparser.ConfigParser:
    """
    Walk through every entry in streamtr.conf and let the user change values.
    ...
    print("")
    print("=" * 55)
    print("  streamtr Configuration Editor")
    print("=" * 55)
